...
**인증은 SSH만 지원합니다.** DBVC는 자격 증명을 묻지도 저장하지도 않고, libgit2가 시스템 `ssh`에 그대로 위임합니다. HTTPS 원격을 매핑하면 Pull이 실패하면서 SSH로 바꾸는 방법을 안내합니다. 준비 절차는 [체크리스트 2단계](docs/setup-checklist.md#2단계--ssh-준비-개발-노트북)에 있습니다.

**데이터베이스 연결은 Windows 통합 인증만 사용합니다.** SQL 인증 계정을 입력할 자리가 없습니다.
