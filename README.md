...
## 주요 기능
- **변경 사항 자동 감지 (DDL Trigger):** DDL 트리거가 스키마 변경을 발생 즉시 `DBVC_ChangeLog` 테이블에 기록합니다. 기록된 변경은 **새로고침**을 누르거나 **연결·DBVC 초기화·Commit**이 끝난 뒤 자동으로 화면에 반영됩니다(주기적 폴링은 하지 않습니다). 새로고침은 이 로그가 가리키는 객체만 추출하므로 객체가 많은 데이터베이스에서도 빠릅니다 — 로그가 모르는 차이를 되찾으려면 **전체 다시 추출**을 쓰세요.
