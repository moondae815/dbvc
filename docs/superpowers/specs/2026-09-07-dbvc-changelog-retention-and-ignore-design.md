...
| 실패 | 결과 | 복구 |
| --- | --- | --- |
| 되돌리기 일부/전부 실패 | **실패한 파일만** 중단된다 — 성공한 파일은 되돌려지고 그 행도 닫힌다. 한 파일이 잠겼다고 나머지 선택 항목까지 되살아나지는 않는다 | 실패 목록에 뜬 파일만 다시 시도한다(다른 프로그램이 열고 있는지 확인) |
| 되돌리기 성공, `MarkProcessed` 실패 | 파일은 깨끗, 행은 열림 → 다음 새로고침에 되살아난다 | 기존 실패 문구가 정확히 이 상황을 설명한다 |

되돌리기가 파일 단위로 성공·실패가 갈리므로 "전부 아니면 전무"로 묶지 않는다 — 실패한 파일의
행만 열어 두고 닫을 수 있는 나머지는 그대로 닫는다. 한 파일이 잠겨 있다는 이유로 선택한 나머지
항목까지 되살리면, 무시가 여러 개를 한 번에 치우는 도구로서 아무 쓸모가 없어진다.

반대 순서(행 먼저, 파일 나중)는 행이 닫힌 채 더러운 파일이 남아 2.2의 표면화로 떠오른다. 복구는
되지만 사용자가 이해할 수 없는 상태다.
