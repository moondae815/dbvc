...
- 출력이 있고 매니페스트가 `0.8.0` → 병합이 먼저 나갔다. 이 작업은 **`0.9.0`**.
- 출력이 없고 매니페스트가 `0.7.2` → 이 작업이 먼저다. 이 작업은 **`0.8.0`**이고, 병합 문서의 버전을 밀어낸다:
  ...
  나온 줄마다 `0.8.0`을 `0.9.0`으로 바꾼다. 단, `0.7.2 → **0.8.0**` 같은 "이전 → 다음" 표기는 `0.8.0 → **0.9.0**`으로 바꾼다(병합 작업 시점의 이전 버전이 이제 0.8.0이다).
- 둘 다 아니면 멈추고 보고한다.
