...
- [ ] **Step 1: `PullResult` enum 파일을 만든다**
...
- [ ] **Step 2: 시그니처를 바꾸고 두 반환문만 옮긴다 (UpToDate 분기는 아직 넣지 않는다)**
...
- [ ] **Step 9: 기존 Core 테스트 두 개의 단언을 옮긴다**
...
- [ ] **Step 3: `Pull()`의 성공 처리를 `switch`로 바꾼다**
...
- [ ] **Step 1: `README.md`의 Pull 설명을 고친다**
...
- [ ] **Step 2: `docs/setup-checklist.md`의 Pull 확인 항목을 고친다**
...
- [ ] `msbuild src/DBVC.Vsix/DBVC.Vsix.csproj -restore -p:Configuration=Release`로 `.vsix`를 만들고, `dir src\DBVC.Vsix\bin\Release\net48\*.vsix`로 **산출물이 실제로 생겼는지** 확인한다(빌드 성공 ≠ .vsix 생성).
- [ ] SSMS 21에 설치하고 Pull을 두 번 누른다. 첫 번째(받을 것이 있을 때)는 저장소 경로가 실린 안내가, 두 번째는 `원격에 새 변경이 없습니다.`가 떠야 한다.
- [ ] 두 번째 Pull 뒤에 이력 탭이 다시 그려지지 않는지 본다.
