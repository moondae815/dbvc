...
- [ ] 이력 탭에서 **분할선을 끌어** 비율을 바꿀 수 있는지
      (전체 이력: 이력 목록 밑과 변경 파일 목록 밑, 두 곳. 특정 객체로 좁힌 뒤에는
      변경 파일 목록이 접히고 **이력 목록⇄Diff 분할선 하나만** 남는 것이 정상이다.
      커밋을 아직 고르지 않아 Diff가 비어 있으면 그 하나도 접힌다)
- [ ] 병합 커밋을 고르면 **머리글**(왼쪽 범위 표시 옆)에
      `병합 커밋입니다 — 첫 부모 기준으로 비교합니다.` 가 뜨는지 — 특정 객체로 좁힌
      상태에서도 뜨는지 (좁힌 이력에는 병합 커밋이 아예 안 나올 수 있다.
      `git log -- <경로>` 와 같은 경로 단순화를 하기 때문이다. 나오는 경우에만 확인한다)
