...
- [ ] 쓸 프로바이더를 정한다. OpenAI 호환 `/chat/completions`를 갖춘 것이면 사내 LLM 서버든
      외부 상용 API든 된다.
- [ ] SSMS에서 **도구 > 옵션 > DBVC > AI 커밋 메시지**를 연다.
- [ ] **프로바이더 주소**에 기준(base) 주소를 넣는다 — **`/v1`에서 끝내야 한다.**
      `/chat/completions`까지 붙은 전체 주소를 넣으면 DBVC가 그 뒤에 `/chat/completions`를
      한 번 더 이어 붙여 경로가 겹치고 404가 난다. 도구가 주소를 대신 고쳐 주지 않으므로
      직접 맞게 적는다. 예: `https://api.openai.com/v1`
- [ ] **모델**·**API 키**(필요하면)·**응답 대기(초)**·**보낼 diff 최대 줄 수**를 채운다.
